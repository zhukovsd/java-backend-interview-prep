...
1. Вычисляется хеш-код объекта
2. Определяется, в какой бакет класть объект
3. Проверяется, пустой ли бакет
4. Если да — элемент добавляется в бакет. Если нет — добавление игнорируется, т.к. HashSet не допускает дубликатов
