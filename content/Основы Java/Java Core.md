...
| **Stack (Стек)**               | Память для хранения локальных переменных, примитивов, ссылок на объекты. Стек организован как LIFO (последним пришёл — первым ушёл). |
...
| **Thread Area (Thread Stack)** | Память для каждого потока, содержит стек вызовов и локальные переменные.                                                             |
