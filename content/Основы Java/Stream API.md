...
|Метод|Тип преобразования|Возвращает|Используется для...|
|---|---|---|---|
|`map()`|**Один ко одному**|`Stream<R>`|Преобразования каждого элемента|
|`flatMap()`|**Один ко многим**|`Stream<R>`|Преобразования + **уплощения** вложенных структур|

🔧 **map(): просто преобразование**

Метод `map()` применяет функцию к каждому элементу потока и возвращает **новый поток** с результатами.
...
Каждое слово превращается в его длину — один вход → один выход.

🧹 **flatMap(): преобразование + уплощение**

Метод `flatMap()` применяется, когда каждый элемент преобразуется в **поток/коллекцию**, и затем все эти потоки **объединяются в один**.
...
List<List<String>> nested = List.of(
    List.of("Java", "Kotlin"),
    List.of("Scala", "Groovy")
);

List<String> flat = nested.stream()
    .flatMap(List::stream)
    .collect(Collectors.toList());
// Результат: [Java, Kotlin, Scala, Groovy]
```

Здесь `flatMap()` сначала превращает каждый вложенный список в поток, а затем **объединяет** их в один.


🧪 **map() vs flatMap() — визуально**

```java
// map()
Stream<List<String>> result = nested.stream().map(list -> list);
// → Stream<List<String>> (вложенный)

// flatMap()
Stream<String> result = nested.stream().flatMap(list -> list.stream());
// → Stream<String> (плоский)
```

🧩 **Когда использовать?**

- ✅ `map()` — когда ты преобразуешь элементы напрямую.
- ✅ `flatMap()` — когда ты работаешь с **вложенными структурами** (списки списков, потоки потоков и т.д.).
...
---
#### 11. Разница между parallel и parallelStream?
...
---
#### 14. Какая разница между findAny() и findFirst()
