...
`CompletableFuture` — это асинхронный контейнер для результата, который может быть вычислен в другом потоке.
Позволяет запускать задачи в фоне и строить цепочки обработчиков без блокирующих операций.
Можно комбинировать несколько future, обрабатывать исключения и управлять потоками.
