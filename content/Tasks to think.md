...
---
 ...
 221. Ситуация. Предположим у тебя есть объект, который представляет из себя List листов стрингов. С помощью Stream API тебе надо найти String  с наибольшей длиной

 222. Ситуация. Есть сервис, который по эндпоинту обновляет что то в базе. В эндпоинт могут одновременно обратиться с разных мест. По умолчанию Spring boot может добавить в базу сразу два одинаковых запроса. В итоге мы ломаем логику. Как решить эту проблему?

 223. Ситуация. Есть standalone провайдер авторизации. KeyClock  к примеру. Как сервис провалидирует этот токен?

 224. Ситуация. Запускаем поток 1. В этом потоке сделаю 5 раз выборку из базу по одному и тому же id. Если включить запросы в БД, что мы увидим?

  225. Ситуация. Если написать дефортный OneToMany? Выберу из базы и закрою транзакцию и попробую сериализовать. Что будет?

 226. Ситуация. У нас нет REST, пришел менеджер, хочет ввести REST. Точно ли нам надо, хорошо ли? Как это сделать?

 227. Ситуация. Есть класс. У него есть статические поля, не статические поля, конструктор. В каком порядке происходит инизиализация?
 
 228. Ситуация. Статический блок, переменная статическая не final, она проинициализированна в блоке инициализации и в самом поле. Какое значение переменная примет?
 
 229. Ситуация. Берем не final переменную. Инициализируем переменную через поле и через конструктор. Какое значение переменная примет?
 
 230. Ситуация. Есть три способа пройтись по коллекции. Расскажи про них и какой в каком случае использовать?
 
 231. Ситуация. Создание потока дорогая операция. Как нам помогает ExecutorService?
 
 232. Ситуация. У многопоточки есть 3 параметра. atomicity, visibility, happens-before. Что из этого нам обеспечивает переменная volatile
 
 233. Ситуация. Как завернуть Java приложение в Docker? Есть JAR. Расскажи пошагово
 
 234. Ситуация. Если я поменяю Service на Component. Что-то поменяется в приложении?
 
 235. Ситуация. Можно миксовать конфигурации? Часть сделать аннотациями, а часть XML?
 
 236. Ситуация. Что если надо заинжектить интерфейс, а у меня две реализации в Context
 
 237. Ситуация. Есть способ проверить в тестах с какими аргументами вызывался метод?
 
 238. Ситуация. Есть две сущности. Связь One to Many. В первой сущности есть поле список второй сущности. Достаем из БД первую сущность и удаляем ее. Что будет с вложенным списком второй сущности
 
 239. Ситуация. Если у нас злоумышленник сворует рефреш токен, он не сможет получить новый access?
 
 240. Ситуация. Есть ArrayList на 10 элементов. Хочу получить доступ к пятому элементу. Почему доступ будет O(1)? Как сработает доступ по индексу? Если в ArrayList лежат strings?
 
 241. Ситуация. Хочу создать LinkedList из strings. Где будут лежать strings?
 
 242. Ситуация. БД. Есть большая таблица. Мы ищем по 3 параметрам и получается очень медленно. Твои действия по оптимизации?
 
 243. Ситуация. REST. Ты сделал Get запрос, как он получает данные? Что тебе надо сделать, чтобы получить определенные данные?
 
 244. Ситуация про отказоустойчивость. Есть старый монолит и новые микросервисы. Старую БД мигрировать не хотим. Если нагрузка повышается, то старый монолит начинает отказывать. Каскадно микросервисы начинают отключаться. Фронт отключился. Как сделать так, чтобы старый микросервис не особо влиял на текущую реализацию? Как выжить в периоды пиковой нагрузки?...Продолжение ситуации. Circuit Breaker  отрубил микросервис, хорошо все заработало. Но нас начали DDOS-ить. Реальные клиенты начинают получать timeout. Что делать?
 
 245. Ситуация. Есть Singleton Bean. Есть Request HTTP запрос. Нам надо из запроса получить хедеры и залогировать. Как можно это заинжектить? Есть ли готовые решения у Spring?
 
 246. Ситуация. Заавтовайрили сервис 1 в сервис 2. В это время сервис 2 требует, чтобы в нем был сервис 3. А сервису 3 нужен сервис 1. Как можно спрвиться с этой ситуацией?
 
 247. Ситуация. Описываешь структуру сотрудника предприятия. У сотрудника есть начальник и подчиненные. Как реализовать такую структуру?
 
 248. Ситуация. Есть огромная таблица на множество столбцов. Как сделать так, чтобы ORM доставало только нужные столбцы?
 
 249. Ситуация. Как написать интеграцию с другими сервисами? С чего начнешь?
 
 250. Ситуация. Предположим поток ничего не выделяет из памяти. Вообще не обращается к Хипу. Этот поток может жить без Стека?
 
 251. Ситуация. Поток который обрабатывает очень большой объекм данных. Поток обращается к данным. Все данные будут закешированы к кеше этого потока?
 
 252. Ситуация. Когда присваиваете значение в переменную это атомарное действие?
 
 253. Ситуация. Если два потока хотят синхронизироваться, какой объект они могут использовать в качестве монитора?
 
 254. Ситуация. Если 10 потоков, которые пришли в synchronize, кто из них первый  попадет в Lock
 
 255. Ситуация. Есть потоки. Вы стартуете на каждую задачу либо сделали threadPool или Executor. У вас ограниченное количество потоков. С точки зрения вычислений это более эффективно или просто удобно?
 
 256. Ситуация. У нас есть некий объект Юзер. Есть три параметра. Как инкапсулировать параметры?
 
 257. Ситуация. Есть класс Parent и от него наследуется класс Child. В Parent есть три метода. Как нам реализовать методы, чтобы ими могли пользоваться только наследуемые. 
 
 258. Ситуация. Есть класс внутри которого находится статический параметр. Если я создам 50 инстрансов этого класса и буду менять этот статический параметр, то что будет?
 
 259. Ситуация. Создал Stream. Произвел операцию. Смогу ли я обратиться к этому стриму еще раз?
 
 260. Ситуация. Есть транзакция, которая состоит из 10 различных комманд. Предположим, что все вычислили правельно, уровень изоляции постален верно. Концепция ACID гарантирует на 100% что проблем не возникнет?
 
 261. Ситуация. Когда делаем JOIN указываем по каким параметрам связываем таблицы. Чаще всего используем оператор ON. Какие еще операторы можно использовать?
 
 262. Ситуация. Что будет если на каждый запрос будет создавать новый ApplicationContext?
 
 263. Ситуация. Что будет если будет исключение в PostConstruct
 
 264. Ситуация. Почему OneToMany Lazy, а ManyToOne Eager
 
 265. Ситуация. Есть Сервис который принимет http запрос и этому Сервису нужно сходить в другой Сервис и принять от него запрос и передать первому Сервису. Запросы через Kafka в синхронном режиме. Код должен принять http запрос и дать синхронно ответ и в промежутке вызвать Kafka.
 
 266. Ситуация. 1000 сообщений в секундку, как нам определить, что пришедший ответ из kafka тот, который нам нужен. 
 
 267. Ситуация. Используем SpringBootApplication, а используемые Бины лежат в библиотеке. Будут ли они подтягиваться автоматически?
 
 268. Ситуация. Maven. Используем библиотеку в которой есть зависимость для БД, но БД мы не используем.  Как частично отключить библиотеку. Полностью отключать библиотеку не нужно. 
 
 269. Ситуация. Есть микросервис, к нему делается REST запрос, который отрабатывает довольно большой промежуток времени. (2-3 минуты) Варианты почему так долго выполняется и варианты устранения и оптимизации.
 
 270. Ситуация. Нужно делать поиск по большому количеству колонок. Не хочу делать индексы. Что можно придумать?
 
 271. Ситуация. Можно в RabbitMQ сообщение послать напрямую или нужно промежуточные сущности вводить?
 
 272. Ситуация. Как можно сравнить объекты между собой кроме equals и hashcode
 
 273. Ситуация. Нужно отсортироваться List с какими то классами. Стринговыми юзерами. а стринговые юзеры они храняться в пакете который мы получили из мавена. Что тут лучше использовать?
 
 274. Ситуация. Если мы напишем stream.filter.filter.filter что произойдет?
 
 275. Ситуация. GC. Есть два объекта. Первый ссылается на второй, а второй ссылается на первый. Удалятся ли эти объекты?
 
 276. Ситуация. Есть Сервис. В Сервисе есть два метода, все методы транзакционные. У второго метода REQUIRES NEW. Первый метод вызывает второй, сколько в итоге будет транзакций?
 
 277. Ситуация. Как без LAZY мы можем сделать ленивую загрузку бина?
 
 278. Ситуация. Есть проект. В проекте есть Kafka и БД. Указываем параметры подключения к БД. Если у нас не указаны переменные в Application Properties для БД, то нам ДатаСоурс создавать не надо. Если у нас не указаны параметры application properties то нас Бин поднимать не надо. Как тогда сделать?
 
 279. Ситуация. Есть два теста. Первый тест, что-то насеттил в базу, второй тест, тоже что-то нассетил в базу. Третий тест пытается, что-то прочитать из базы. Какая тут проблема?
 
 280. Ситуация. Есть тест. Нужно инициализировать БД, чтобы прогнать тест. Как можно выполнить SQL скрипт перед выполнением теста?
 
 281. Ситуация. Есть Сервис, а в сервисе есть метод. Метод внутри себя использует два Сервиса и два Репозитория. Как будем тестировать этот Сервис?
 
 282. Ситуация. Если ловим OptimisticLockException, что делаем?
 
 283. Ситуация. Есть формочка поиска в интернет магазине и надо строить динамический sql запросы в зависимости от параметров с формы. Как мы можем по запросу формировать динамические sql запросы?
 
 284. Ситуация. Берем постгре консоль, где можно выполнять запросы. Нужно вывести первую страницу, которая содержит пять элементов. Как должен выглять запрос?
 
 285. Ситуация. Находимся на первой страничке интернет магазина. Вывелось пять элементов. Переходим на вторую страницу, но в момент перехода добавляется еще пять элементов. Получается, что при переходе с первой страницы на вторую мы увидем информацию с первой страницы. Как решить?
 
 286. Ситуация. Есть колонка с полом. Мужской и женский. Если повесим индекс, как ты думаешь будет высокая селективность?
 
 287. Ситуация. Если у индекса низкая селективность, то он используется в выборке?
 
 288. Ситуация. Приложение супер оптимизировано, но мы уперлись в производительность в БД. Как это можно решить?
 
 289. Ситуация. Что делать есть в приложении один бин имеет разные реализации?
 
 290. Ситуация. Есть маленькое приложение, которое использует два бина. Бин А это синглетон инжектит в сябя бин Б. Бин Б это прототайп. Сколько у нас бинов будет?
 
 291. Ситуация. Есть самописная аннотация. Есть метод, который реализует определенный функционал. Как сделать так, чтобы когда мы ставили нашу самописную аннотацию вызывался наш метод с прописанным функционалом. 
 
 292. Ситуация. Есть два таблицы.  Отделы и сотрудники. Есть связь Один ко многим. Надо выбрать id всех отделов где количество сотрудников больше 10.
